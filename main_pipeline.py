...
def main(image:str="BDI-Nuclei_Segmentation\data_jpeg\GTEX-113JC-2226.jpg", w_target:int=55750, h_target:int=40500, tile_size:int=250):
    print('main called')
    ...
    for i in range(10):
        chosen_tile = tiles[tissue_tiles[i*5]]
        ...
        # Save images
        Image.fromarray(mask_to_save).save(f"./BDI-Nuclei_Segmentation/results/mask{i}.jpg", format="JPEG")
        Image.fromarray(tile_to_save).save(f"./BDI-Nuclei_Segmentation/results/tissue_tile{i}.jpg", format="JPEG")
    
        dice_coefficient = segmentation_evaluate(mask, chosen_tile, i)
        print('dice: ', dice_coefficient)
